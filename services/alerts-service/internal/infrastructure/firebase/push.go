...
// SendAlert sends a push notification to a device FCM token.
func (n *PushNotifier) SendAlert(ctx context.Context, fcmToken, title, body string, data map[string]string) error {
	if fcmToken == "" {
	...
	n.logger.Info("push notification sent", zap.String("messageId", msgID))
	return nil
}
