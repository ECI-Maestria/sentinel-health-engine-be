// Package postgres implements repository ports using PostgreSQL via pgx.
package postgres
...
// Connect opens a connection pool to PostgreSQL and applies pending migrations.
// It reads the CALENDAR_DATABASE_URL environment variable for the DSN.
func Connect(ctx context.Context, logger *zap.Logger) (*pgxpool.Pool, error) {
	dsn := os.Getenv("CALENDAR_DATABASE_URL")
	if dsn == "" {
		return nil, fmt.Errorf("CALENDAR_DATABASE_URL env var is required")
	}
