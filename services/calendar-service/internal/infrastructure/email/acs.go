...
// NewSender builds the sender from ACS_CONNECTION_STRING env var.
// Connection string format: endpoint=https://...;accesskey=Base64Key==
func NewSender(logger *zap.Logger) (*Sender, error) {
...
// Send sends an HTML email to recipientEmail.
func (s *Sender) Send(ctx context.Context, recipientEmail, subject, htmlBody string) error {
	if recipientEmail == "" {
...
func (s *Sender) signRequest(req *http.Request, body []byte) error {
